<template>
  <div
    class="bg-white rounded-[3.3rem] p-10 w-[320px] h-[320px] flex flex-col items-center justify-center shadow-xl"
  >
    ...
    >
      <div class="flex items-center gap-4 bg-[#E5E5EA] relative w-full justify-between">
        <div
          :class="`absolute left-0 right-0 h-full rounded-r-2xl bg-white transition-all duration-700 ease-in-out`"
        ...
        <button
          ...
          class="p-3 py-5 h-full flex items-center justify-center text-2xl font-bold z-10 text-[#3C3C43] cursor-pointer"
        >
          <svg class="w-6" viewBox="0 0 65 11" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
        ...
        >
          <svg class="w-6" viewBox="0 0 63 66" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
    ...
    <div class="flex w-full justify-between text-[#3C3C43]/50 font-semibold text-sm mt-2">
      <span>${{ min }}</span>
      <span>${{ max }}</span>
    </div>
  </div>
...
let decrementTimer: ReturnType<typeof setInterval> | null = null
...
const stopDecrement = () => {
  ...
  if (decrementTimeout) clearTimeout(decrementTimeout)
